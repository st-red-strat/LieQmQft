...
物理学では通常、予測したい物理量を未知変数とする微分方程式を立て、
これを解くことで物理現象を予言する。
この際、目的（多くの場合、手計算またはコンピュータによる数値計算で問題を解くこと）のために
問題を簡単化する事が頻繁に行われる。

以下では、我々の日常スケールであるボールの放物運動であるとか、
電磁場中の荷電粒子の運動、ばねによる復元力が質量を持つ物体に働いている場合（調和振動子）などの物理系を考えるが、
実際には物体が大きさを持っていたり、回転したり、変形することを一切無視して、
物体を質量だけを持ち大きさが無限小の点と見做す。
このような点を質点といい、物体に複雑な力が働いていない場合は大抵よい近似を与える。
もちろん後に学ぶ解析力学は質量を持ち、大きさが有限であるような物体
（この中で特に変形を無視したものを剛体と呼ぶ）に対しても適用可能であるが、
出発点として質点の力学を理解することはよい学びを与えるであろう。
...
運動エネルギー（kinetic energy）$T$と
ポテンシャルエネルギー（potential energy）$V$
が
...
$n$次元調和振動子
\footnote{
  単に$1$次元調和振動子を$n$個連ねたもの。
  特に、$m := m_1 = m_2 = \dots = m_n$の場合を等方調和振動子、
  それ以外の場合を非等方調和振動子などと呼ぶ場合があるが、
  特に断りなく$n$次元調和振動子と言った場合は等方調和振動子を指すことが多い。
}
の場合Lagrangianは、
第$j$番目の質点の
運動エネルギーが質量$m_j$を用い
$T_j = \dfrac{m_j}{2} \dot{q}_j^2$
ポテンシャルエネルギーが
$V_j = \dfrac{m_j \omega_j^2}{2} q_j^2$
と与えられることから
\begin{align}
  L = T - V
= \sum_{j=1}^n
  \bigg(
    \dfrac{m_j}{2}\dot{q}_j^2
  -
    \dfrac{m_j \omega_j^2}{2} q_j^2
  \bigg)
