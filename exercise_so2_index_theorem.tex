...
$U_\theta$ の単位元（$\theta=$?）と逆操作 $U_\theta^{-1}$ を答えよ。
...
$n\ge0$ が普通のべき，$n<0$ がその逆数）の基底になっている。この基底について\textbf{正しいものをすべて選べ}。
\begin{itemize}\setlength{\itemsep}{1pt}
\item[(あ)] $\{z^n=e^{in\varphi}\}_{n\in\mathbb{Z}}$ は円周上の関数の Fourier（整数べき／Laurent）基底である。
\item[(い)] 各 $z^n$ は回転 $U_\theta$ の固有関数（固有値 $e^{-in\theta}$）で，\emph{回転の固有関数がそのまま基底}である。
\item[(う)] 有限個の $z^n$（$n=0,\dots,N$）だけで円周上の任意の関数を厳密に表せる。
\end{itemize}
\end{enumerate}
\end{prob}
...
$(X,Y)$ の原点での指数として読む。\textbf{ただし矢印（ベクトル場）も向きだけでなく変換する}：場を
流れの速度 $\dot z=P+iQ=:f(z)$ とみなすと，$Z=1/z$ では $\dot Z=-\dot z/z^2=-Z^2 f(1/Z)$ になる。
この $\dot Z$ の $Z=0$ での\textbf{零点の位数}が $\infty$ での指数である（$\dot z\to\dot Z$ の変換で位数が
$2$ だけ増える点に注意。これを落とすと総和が合わない）。

\begin{prob}
\begin{enumerate}[start=8]
\item \textbf{指数の総和。}次の各ベクトル場について，平面内の零点と無限遠点 $\infty$ での指数をすべて求め，
その総和を答えよ。（複素数で書くと $\bm v\leftrightarrow$（$P+iQ$）で，零点の指数 $=$ そこでの
$P+iQ$ の零点の位数になる。$\infty$ では上のとおり $\dot Z=-Z^2 f(1/Z)$（$f=P+iQ$）に直してから
$Z=0$ での零点の位数を読む。）
\[
\begin{aligned}
...
各 $\psi_k$ の固有値 $j_k$ を書き下せ（境界の回転演算子の固有関数は整数べき $z^k=e^{ik\varphi}$，
固有値 $k-\tfrac n2$）。
\item \textbf{符号の非対称性。}赤道上の固有値の集合 $\{j_k=k-\tfrac n2:k=0,\dots,n\}$ について，
\emph{正の個数 $N_+$（$j_k>0$）}，\emph{負の個数 $N_-$（$j_k<0$）}，\emph{零の個数 $h$（$j_k=0$）}を
$n$ の偶奇で求めよ。さらに，$k$ を全整数に広げた境界演算子での「符号の非対称性」
$\displaystyle\sum_k\operatorname{sign}(j_k)$（この量を $\eta$ と書く；くわしい定義は補足）の値を，スペクトルが
$0$ について対称であることを使って答えよ。
...
指数 $n+1$ が変わるか否かを答えよ（状態が南の勘定から北の勘定へ移るだけ）。
\end{enumerate}
\end{enumerate}
\end{prob}
