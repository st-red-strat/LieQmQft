...
\textit{(d)} 正しいのは $\boxed{\text{(あ)(い)(え)}}$（(う) は誤――以下のとおり最短経路が支配するのは
特殊な場合のみで，無偏り拡散では菱形にならない）。確率は\emph{最短経路の長さ}（``エネルギー''）ではなく\emph{経路数＝多重度}
...
\textit{(a)} 正しいのは $\boxed{\text{(あ)(い)}}$。(あ)：左右・上下対称ゆえ $\langle\Delta\bx\rangle=\bm0$ で 1 階差分
（移流項 $\partial_x$）が相殺し，最低次に残るのは 2 階差分 $\partial_x^2$。(い)：$\langle\Delta x^2\rangle\sim a^2$ と時間刻み
$\tau$ が $\tau\propto a^2$ で釣り合う（$\tau\partial_tP\sim a^2\nabla^2P$）のが「時間 1 階・空間 2 階」の由来。
(う) は誤――空間 2 階は歩みの対称性（$\langle\Delta x\rangle=0$）に由来し，1 次元の対称歩でも 2 階になる。$C_4$ は
等確率面の等方性（$x^2+y^2$）を保証するが，微分が 2 階である理由ではない。

\textit{(b)} 偏り $\langle\Delta x\rangle\sim a\neq0$ があると 1 階差分が残り，主要項は空間\textbf{1 階}の移流項 $\sim a\,\partial_xP$。
バランス $\tau\partial_t\sim a\,\partial_x$ より $\boxed{\tau\propto a^{1}}$，方程式は\textbf{移流型} $\boxed{\partial_tP=-v\,\partial_xP}$
（$v=\lim\langle\Delta x\rangle/\tau$）。答：\textbf{階数 $1$，$\tau\propto a^1$，移流型}。拡散項（$\sim a^2\nabla^2/\tau\sim a\to0$）は
subleading で，拡散を見るには移動座標系で残差に diffusive scaling を施す。\emph{消えない最低次の変位モーメントの
次数が空間微分の階数と $\tau$ の冪を決める}――1 次なら $\tau\sim a$ で移流，2 次なら $\tau\sim a^2$ で拡散。
\end{sol}
...
\]
を満たす。すなわち実効拡散定数は $\boxed{D_x=D}$（不変），$m$ 依存の減衰率は
$\boxed{\Gamma_m=\dfrac{Dm^2}{R^2}}$。（``質量''$\mu_m=m/R$ で $\Gamma_m=D\mu_m^2$。）
...
\par\smallskip
\textbf{(17) 考察。}正しいのは $\boxed{\text{(あ)(い)(う)(え)}}$。

\textit{(あ)} 指数 $e^{-Dm^2t/R^2}$（モード和）と $e^{-(2\pi Rw)^2/4Dt}$（像和）は無次元量 $Dt/R^2$ について
互いに逆。$t\gg t^\ast$ ではモード和が $m=0$ の 1 項＝(14)、$t\ll t^\ast$ では像和が $w=0$ の 1 項＝(15)。
速く収束する側が極限ごとに入れ替わる（Poisson 和の双対性）。

\textit{(い)(う)} $\Gamma_m=Dm^2/R^2=D\mu_m^2$（$\mu_m=m/R$）で KK 質量 $m/R$ に対応。逆時間 $1/t$ が
エネルギースケール、$1/t^\ast=D/R^2$ がコンパクト化スケール。低エネルギー（長時間）ではゼロモードのみで
$d_{\mathrm{eff}}=1$、高エネルギー（短時間）では塔全体が効き $2$ が回復（(16) の $d_{\mathrm{eff}}:2\to1$）。

\textit{(え)} (13) の双対は theta 関数のモジュラー変換（Poisson 和）で，モジュラー的パラメータは
無次元 $Dt/R^2$。運動量 $m$（間隔 $1/R$）と巻きつき $w$（間隔 $2\pi R$）を入れ替える双対では
$R\to\tilde R\sim Dt/R$（$R\tilde R\sim Dt$、拡散長二乗 $Dt$ が $\alpha'$ に相当）、自己双対点 $R^2\sim Dt$（$t\sim t^\ast$）。

\textit{(お) は誤}――短時間 $t\ll t^\ast$ では像和の $w=0$ 項が支配し，コンパクト性が\emph{見えなくなって}
自由 2 次元拡散に帰着する（1 次元になるのは長時間側）。極限を取り違えた誤答。
\end{sol}
