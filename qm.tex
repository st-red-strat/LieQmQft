...
Bell inequalityの破れにより今では実験的に否定されている。
物理的に明確に区別できる
...
この他に考えている粒子が持つspin自由度を考える必要もあるが、
これを理解するためには先に
...
カシミール演算子（Casimir operator、quadratic Casimir invariant）と呼ばれる
...
\end{align}
のように非負であるため、
無次元の実数parameter $j \ge 0$を用いて
$L^2 = \hbar^2 j (j+1)$
とおいて一般性を失わない。
同様に無次元parameter $m$を用い、
